...
};      // bussiness information structure
vector<BusinessInfo> g_bussiness; // <src,dest>

struct channel_state{
    vector<LL> used_edges;
    LL total_remaining = 0;
    LL used_edges_num = 0;
};   // strucure of the channel's state
...
void CreateGraph()
{
    ...
    for (LL i = 0; i < g_M; i++)
    {
        LL s, t, d;
        cin >> s >> t >> d;
        g_graph.addEdge(s, t, d, g_P, i);
        // g_graph.addEdge(t, s, d, g_P, i); // 反方向再存储一次
    }
    // business
    for(LL i = 0; i < g_T; i++)
    {
...
void DijsktraResult(BussinessResult& br_, LL index_)
{
    ...
    for(LL i = 0; i < br_.path_edges_.size(); i++)
    {
        bussiness_result.push_back(br_.path_edges_[i]);
    }
    for(LL i = 0; i < br_.larger_pos_.size(); i++)
    {
        bussiness_result.push_back(br_.larger_pos_[i]);
    }
...
int DijkstraCompare(Graph& graph_, LL src_, LL dest_, unordered_map<LL, vector<LL>> parent_,\
        ...
        for (auto e : graph_.adj[node]) 
        {
            ...
            if (e.index == parent_[node][2])
            {
                ...
                for(LL i = 0; i < g_P; ++i)
                {
                    ...
                    if(e.channels[i])
                    {
                        channels_used_[i].total_remaining += e.remaining;
                        channels_used_[i].used_edges.push_back(e.index);
                        channels_used_[i].used_edges_num += 1;
                    }
                }
                break;
            }
    ...
    while (node != src_)
    {
        ...
        for (auto e : graph_.adj[node]) 
        {
            ...
            if (e.index == parent[node][2])
            {
                for(LL i = 0; i < g_P; ++i)
                {
                    if_used_channel[i] = if_used_channel[i] | e.channels[i];
                }
                break;
            }
        }
